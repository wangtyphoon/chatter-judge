...
async def print_submissions():
    try:
        ...
        df = pd.DataFrame(
            [
                [submission.id, submission.user.username, submission.question.scope.name, 
                 submission.question.name, submission.status, submission.created_at] 
...
async def draw_race_bar(selected_scope, selected_question):
    try:
        ...
        if not df.empty:
            ...
            df["Time"] = df["Time"].dt.floor('s')
            df["Time"] = df["Time"].dt.tz_convert('Asia/Shanghai')
            df = df[(df['Scope'] == selected_scope) & (df['Question'] == selected_question)&(df['Status']==0)]
